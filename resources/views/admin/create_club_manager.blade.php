...
<div class="max-w-md mx-auto py-8">
    <div class="bg-white shadow rounded-lg p-8">
        <h2 class="text-2xl font-bold mb-2">Add Club Manager</h2>
        <p class="mb-6 text-gray-600">Register a new club manager for your organization.</p>
        @if(session('success'))
            <div class="mb-4 p-3 bg-green-100 text-green-800 rounded border border-green-200">
                {{ session('success') }}
            </div>
        @endif
        <form method="POST" action="{{ route('admin.users.create-club-manager') }}">
            @csrf
            <div class="mb-4">
            ...
            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 font-semibold w-full">Add Club Manager</button>
        </form>
    </div>
</div>
@endsection
